...
export function customSimulationSystemPrompt(): string {
  return [
    ...
    "",
    "DESIGN RULES:",
    "1. Total seated work time ~110–120 minutes. Minutes per phase must sum to 110–120.",
    "2. Exactly 4 sequential phases. Each phase: read artifacts → decide → write ONE text deliverable in the response box.",
    ...
    "}",
    "",
    "Quality bar: artifacts look like real inbox/Slack/docs; every deliverable is finishable in the text box using only on-screen materials.",
  ].join("\n");
}

export function customSimulationUserPrompt(p: CustomSimulationParams): string {
  return [
    "Build a near-2-hour interactive competency simulation as JSON.",
    ...
    "- Phases must sum to 110–120 minutes.",
    "",
...
export function evaluationSystemPrompt(): string {
  return [
    ...
    "The simulation may be multi-phase (~2 hours of work). The candidate response may include labeled sections (PHASE 1, PHASE 2, etc.).",
...
export function evaluationUserPrompt(params: {
  ...
  return [
    `Role: ${params.role}`,
    "",
    "Simulation prompt (multi-phase; may span ~2 hours of work):",
